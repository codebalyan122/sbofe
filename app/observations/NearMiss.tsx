// screens/NearMissObservation.tsx
...
export default function NearMissObservation() {
  const { getAuthHeaders, user } = useAuthStore();
  const { addRecentActivity } = useObservationStore();
  const router = useRouter();
  const scrollViewRef = useRef<ScrollView>(null);

  // Hardcoded type for Near Miss Observation
  const observationType = 'nearmiss';
  ...
  const [formData, setFormData] = useState({
    ...
    unit: 'Unit 1',
    area: '',
    areaManager: '',      // Will be populated by selectedManagerName
    category: '',
    subCategory: 'General',
    description: '',
    immediateAction: '',
    selectedManager: '', // Manager ID
    selectedManagerName: '', // Manager Name for display
    images: [] as ImageData[],
    reportedBy: safeText(user?.username, 'Observer'),
    observerName: safeText(user?.username, 'Observer'),
  ...
  const fetchAreaManagers = useCallback(async () => {
    ...
    try {
      console.log('Fetching area managers from:', `${baseUrl}/api/area-manager-dashboard/managers`);
      const response = await fetch(`${baseUrl}/api/area-manager-dashboard/managers`, {
        method: 'GET',
      ...
      console.log('Area managers fetched:', data);
      ...
      Alert.alert('Error', 'Failed to load area managers. Please try again.');
      setAreaManagers([]);
    } finally {
  ...
  const autoAssignAreaManager = useCallback((selectedArea: string) => {
    ...
    if (predefinedManager) {
      ...
      setFormData(prev => ({
        ...
        areaManager: predefinedManager.name
      }));
      return;
    }
    ...
    if (assignedManager) {
      ...
      setFormData(prev => ({
        ...
        areaManager: safeText(assignedManager.username, 'Unknown Manager')
      }));
    } else {
  ...
  const handleManagerSelect = (managerId: string) => {
    ...
    setFormData(prev => ({
      ...
      selectedManagerName: selectedManager ? selectedManager.username : '',
      areaManager: selectedManager ? selectedManager.username : ''
    }));
  ...
  // Near miss observation specific categories and subcategories with proper typing
  ...
  const getCategoryColor = (category: string, isSelected: boolean = false) => {
    ...
    if (isSelected) {
      return getTypeColor(); // Use near miss orange for selected
    }
  ...
  // Near miss observation colors and styling
  ...
  const uploadSingleImage = async (imageData: ImageData) => {
    ...
    try {
      console.log('Uploading to:', `${baseUrl}/api/nearmiss/upload`);
      const response = await fetch(`${baseUrl}/api/nearmiss/upload`, {
        method: 'POST',
        body: formDataUpload,
        headers: {
  ...
  const saveToDatabase = async () => {
    try {
      console.log('Saving near miss report to database with data:', formData);
      ...
      const submissionData = {
        ...
        unit: formData.unit,
        areaManager: formData.selectedManagerName || formData.areaManager,
        area: formData.area,
        category: formData.category,
        subCategory: formData.subCategory,
        description: formData.description,
        immediateAction: formData.immediateAction,
        selectedManager: formData.selectedManager, // Manager ID
      ...
      console.log('observerName:', submissionData.observerName);
      console.log('location:', submissionData.location);
      console.log('unit:', submissionData.unit);
      console.log('areaManager:', submissionData.areaManager);
      console.log('category:', submissionData.category);
      console.log('subCategory:', submissionData.subCategory);
      console.log('description:', submissionData.description);

      console.log('Full submission data:', submissionData);
      console.log('submitting to:', `${baseUrl}/api/nearmiss/save-sbo`);

      const response = await fetch(`${baseUrl}/api/nearmiss/save-sbo`, {
        method: 'POST',
        headers: {
      ...
      if (result.success && (result.observation || result.data)) {
        ...
        const activityData = {
          ...
          type: 'nearmiss' as const,
          title: `Near Miss - ${formData.category}`,
          description: formData.description,
          location: formData.location,
          category: formData.category,
        ...
        console.log('➕ [NEAR MISS] Adding to recent activities:', activityData);
        addRecentActivity(activityData);
      } else {
        // Fallback if server doesn't return the observation
        console.log('⚠️ [NEAR MISS] No observation data in response, creating fallback');
  ...
  // Enhanced handleNext with manager validation
  const handleNext = () => {
    if (currentStep === 1) {
      // Validate location
  ...
  // Enhanced submit handler with manager validation
  const handleSubmit = async () => {
    ...
    if (!formData.observerName?.trim()) missingFields.push('Observer Name');
    ...
    // Area manager is now optional - removed from required validation
    ...
    // If all required fields are present, show submit confirmation
    Alert.alert(
      'Submit Near Miss Report',
      'Are you sure you want to submit this near miss incident report?\n\nThis will be sent to the safety team for investigation and preventive action.',
      [
        {
          text: 'Cancel',
          style: 'cancel',
        },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(
                'Near Miss Reported!',
                'Your near miss incident report has been submitted successfully.',
                [
                  {
                    text: 'Done',
                    onPress: () => router.back()
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 1:
        return (
          <View style={styles.stepContent}>
            ...
            <Text style={styles.stepSubtitle}>Choose where this near miss incident occurred</Text>
                  ...
                  {getCategorySubcategories(formData.category).map((subcategory: string) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => setFormData(prev => ({ ...prev, subCategory: subcategory }))}
                        activeOpacity={0.8}
                      >
                        <Text style={[
                          ...
                          <View style={styles.managerInfo}>
                            <View style={styles.managerAvatar}>
                              <Text style={styles.managerAvatarText}>
                                {manager.username ? manager.username[0].toUpperCase() : '?'}
                              </Text>
                            </View>
                            <View style={styles.managerDetails}>
                              <Text style={styles.managerName}>{manager.username}</Text>
      ...
      case 3:
        return (
          <View style={styles.stepContent}>
            <Text style={styles.stepTitle}>Near Miss Incident Details</Text>
            <Text style={styles.stepSubtitle}>Describe the near miss incident and response actions</Text>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholder="Describe what happened, what could have gone wrong, and the potential consequences if an injury had occurred..."
            ...
            <View style={styles.inputGroup}>
              <Text style={styles.inputLabel}>Response Action Taken</Text>
              <TextInput
                style={[
                  styles.textInput,
                  styles.textArea,
                ...
                placeholder="Describe any immediate response actions taken and preventive measures implemented..."
            ...
            <View style={styles.inputGroup}>
              ...
              {formData.images.length === 0 ? (
                <TouchableOpacity style={styles.noImagesContainer} onPress={showImagePicker}>
                  ...
                  <Text style={styles.noImagesSubtext}>Tap to add photos of the near miss incident area</Text>
                </TouchableOpacity>
              ) : (
      ...
      case 4:
        return (
          <View style={styles.stepContent}>
            ...
            <Text style={styles.stepSubtitle}>Review your near miss incident report before submitting</Text>
            ...
            <View style={styles.reviewCard}>
              <View style={styles.reviewHeader}>
                ...
                <View style={styles.reviewTypeInfo}>
                  <Text style={styles.reviewTypeTitle}>NEAR MISS INCIDENT</Text>
              ...
              <View style={styles.reviewSection}>
                ...
                {formData.sublocation ? <Text style={styles.reviewSectionSubtext}>{formData.sublocation}</Text> : null}
                {formData.area ? <Text style={styles.reviewSectionSubtext}>{formData.area}</Text> : null}
              </View>
              ...
              {formData.subCategory && formData.subCategory !== 'General' && (
                <View style={styles.reviewSection}>
              ...
              {formData.selectedManagerName && (
                <View style={styles.reviewSection}>
                  ...
                  <Text style={styles.reviewSectionText}>{formData.selectedManagerName}</Text>
                </View>
              )}
              ...
              {formData.immediateAction && (
                <View style={styles.reviewSection}>
                  <Text style={styles.reviewSectionTitle}>Response Action Taken</Text>
              ...
              <View style={styles.reviewSection}>
                ...
                <Text style={styles.reviewSectionText}>{formData.reportedBy}</Text>
              </View>
            </View>
          </View>
        );
      ...
      >
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerTitleContainer}>
            ...
            <Text style={styles.headerSubtitle}>Incident Prevention Report</Text>
          </View>
            ...
            {currentStep < 4 ? (
              <TouchableOpacity
                style={[
                  ...
                  {
                    backgroundColor: getTypeColor()[0],                    
                    opacity: ((!formData.location || (getLocationSubLocations(formData.location).length > 0  && !formData.sublocation) || !formData.area) && currentStep === 1) ||
                      ((!formData.category ||
                        (getCategorySubcategories(formData.category).length > 0 && !formData.subCategory) ||
                        !formData.selectedManager) && currentStep === 2) ||
                      (!formData.description && currentStep === 3) ? 0.6 : 1
                  }
                ]}                
                onPress={handleNext}
                disabled={((!formData.location || !formData.area) && currentStep === 1) ||
                  ((!formData.category ||
                    (getCategorySubcategories(formData.category).length > 0 && !formData.subCategory) ||
                    !formData.selectedManager) && currentStep === 2) ||
